    ...
    public void configure() {
        ...
        from("direct:postSendMoney").routeId("com.modusbox.postSendMoney")
            ...
            .to("bean:customJsonMessage?method=logJsonMessage('info', ${header.X-CorrelationId}, " +
                    ...
                    "POST {{ml-outbound.endpoint}}', " +
                    "'Tracking the request', 'Track the response', 'Input Payload: ${body}')")

            .toD("{{ml-outbound.endpoint}}/transfers?bridgeEndpoint=true")
            .unmarshal().json()
...
//            .to("direct:putTransfersAcceptParty")
        ...
        from("direct:putTransfersAcceptParty")
                ...
                .to("bean:customJsonMessage?method=logJsonMessage('info', ${header.X-CorrelationId}, " +
                        ...
                        "PUT {{ml-outbound.endpoint}}/transfers/${exchangeProperty.postSendMoneyInitial?.get('transferId')}', " +
                        "'Tracking the request', 'Track the response', 'Input Payload: ${body}')")
//                .marshal().json()
                // Instead of having to do a DataSonnet transformation
                .toD("{{ml-outbound.endpoint}}/transfers/${exchangeProperty.postSendMoneyInitial?.get('transferId')}?bridgeEndpoint=true")
                .unmarshal().json()
                .to("bean:customJsonMessage?method=logJsonMessage('info', ${header.X-CorrelationId}, " +
...
//                .process(exchange -> System.out.println())
                ...
                .to("bean:customJsonMessage?method=logJsonMessage('info', ${header.X-CorrelationId}, " +
                        ...
                        "'Request sent to PUT {{ml-outbound.endpoint}}/transfers/${header.transferId}, with body: ${body}')")
//                .process(exchange -> System.out.println())                        
                .toD("{{ml-outbound.endpoint}}/transfers/${header.transferId}?bridgeEndpoint=true")
                .unmarshal().json()
                .to("bean:customJsonMessage?method=logJsonMessage('info', ${header.X-CorrelationId}, " +
